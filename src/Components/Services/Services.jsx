...
const Services = () => {
  ...
  const [tabLoading, setTabLoading] = useState(false); // loading for specific tab switch
  ...
  useEffect(() => {
    ...
    setTabLoading(true);
    const timeout = setTimeout(() => setTabLoading(false), 300); // Simulate loading
    return () => clearTimeout(timeout);
  }, [activeTab]);

  const filteredData = busData.filter(
    (bus) =>
      bus.company.toLowerCase() === tabs[activeTab].company.toLowerCase()
  );
